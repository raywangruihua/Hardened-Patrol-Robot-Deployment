...
#include <opencv2/opencv.hpp> // Modification: Added for blur and DNN inference
...
class UVCCameraDriver {
 ...
 private:
  ...
  // Modification: Added setupYOLO() declaration
  void setupYOLO();
 ...
 private:
  ...
  // Modification: Added blur_ and selective_blur_ flag pointers and DNN net_
  std::atomic<bool>* blur_ = nullptr;
  std::atomic<bool>* selective_blur_ = nullptr;
  cv::dnn::Net net_;
};
