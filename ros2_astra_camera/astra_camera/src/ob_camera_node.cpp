...
void OBCameraNode::setupUVCCamera() {
  if (use_uvc_camera_) {
    ...
    // Modification: Pass blur_ and selective_blur_ flags to the UVC driver
    uvc_camera_driver_ =
        std::make_shared<UVCCameraDriver>(
          node_, parameters_, color_camera_info, serial_number, 
          &blur_, &selective_blur_);
  } else {
...
void OBCameraNode::setupPublishers() {
  ...
  // Modification: Added /voice listener cycling through off, full blur and selective blur
  blur_sub_ = node_->create_subscription<std_msgs::msg::Int32>(
      "/voice", 1, [this](const std_msgs::msg::Int32::SharedPtr msg) {
        if (msg->data == 6) {
          if (blur_ == true) {
            blur_ = false;
            RCLCPP_INFO_STREAM(logger_, "Removing blur filter.");
          }
          else if (selective_blur_ == true) {
            selective_blur_ = false;
            blur_ = true;
            RCLCPP_INFO_STREAM(logger_, "Switching to blur filter.");
          }
          else {
            blur_ = true;
            RCLCPP_INFO_STREAM(logger_, "Applying blur filter.");
          }
        }
        else if (msg->data == 7) {
          if (selective_blur_ == true) {
            selective_blur_ = false;
            RCLCPP_INFO_STREAM(logger_, "Removing selective blur filter.");
          }
          else if (blur_ == true) {
            blur_ = false;
            selective_blur_ = true;
            RCLCPP_INFO_STREAM(logger_, "Switching to selective blur filter.");
          }
          else {
            selective_blur_ = true;
            RCLCPP_INFO_STREAM(logger_, "Applying selective blur filter.");
          }
        }
      });
}
...
void OBCameraNode::onNewFrameCallback(const openni::VideoFrameRef& frame,
  ...
  cv::Mat publish_image = (stream_index == DEPTH) ? scaled_image : image;
