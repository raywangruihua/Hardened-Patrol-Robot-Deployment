...
/*                                                                        */
/* Modified by Ray Wang Rui Hua, 2026-07-22                               */
/*                                                                        */
/**************************************************************************/
...
// Modification: Added ament index include to resolve the model path from package share
...
// Modification: Added BLUR_AMT constant for the gaussian blur kernel size
const cv::Size BLUR_AMT{51, 51};
...
// Modification: Added setupYOLO() to load the YOLOv8 face model
// models are stored at ros2_astra_camera/astra_camera/models
void UVCCameraDriver::setupYOLO() {
  std::string pkg_path = ament_index_cpp::get_package_share_directory("astra_camera");
  net_ = cv::dnn::readNetFromONNX(pkg_path + "/models/yolov8n-face-lindevs.onnx");
  net_.setPreferableBackend(cv::dnn::DNN_BACKEND_CUDA);
  net_.setPreferableTarget(cv::dnn::DNN_TARGET_CUDA);
  RCLCPP_INFO_STREAM(logger_, "YOLO model initialised.");
}
...
void UVCCameraDriver::frameCallback(uvc_frame_t* frame) {
  ...
  // Modification: Added full-frame gaussian blur gated on blur_
  // apply gaussian blur over the entire video feed
  if (blur_ && blur_->load()) {
    auto cv_image_ptr = cv_bridge::toCvCopy(image);
    cv::GaussianBlur(cv_image_ptr->image, cv_image_ptr->image, BLUR_AMT, 0);
    image = *(cv_image_ptr->toImageMsg());
  }

  // Modification: Added YOLOv8 face detection with selective blur gated on selective_blur_
  // use yolov8n-face-lindevs to find bounding boxes where there are faces
  // then apply gaussian blur over those boxes
  if (selective_blur_ && selective_blur_->load()) {
    auto cv_image_ptr = cv_bridge::toCvCopy(image);
    cv::Mat& frame = cv_image_ptr->image;

    cv::Mat blob = cv::dnn::blobFromImage(
      frame, 1.0 / 255.0, cv::Size(640, 640), cv::Scalar(),
      true, false  
    );
    ...
    // YOLOv8 output is [1, 5, 8400]; reshape to [5, 8400] then transpose
    // to [8400, 5] so each row is a detection (cx, cy, w, h, score)
    ...
    for (int i = 0; i < out.rows; i++) {
      if (out.at<float>(i, 4) < 0.5f) continue;
      float cx = out.at<float>(i, 0) * x_scale;
      float cy = out.at<float>(i, 1) * y_scale;
      float w  = out.at<float>(i, 2) * x_scale;
      float h  = out.at<float>(i, 3) * y_scale;
      cv::Rect box(
      ...
      if (box.area() > 0)
        cv::GaussianBlur(frame(box), frame(box), BLUR_AMT, 0);
    }
